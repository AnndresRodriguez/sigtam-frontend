<template>
  <div class="col-lg-12 grid-margin stretch-card">
      ...
      <div class="card-body">
        <div class="d-flex flex-row justify-content-between">
          ...
          <button
            ...
            data-toggle="modal"
            data-target="#modalMecanicoNuevo"
            @click="cleanData()"
          >
            <i class="mdi mdi-account-plus"></i>Añadir Nuevo Mecánico
          </button>
        </div>
          ...
          <table class="table table-hover">
            ...
            <tbody>
              <tr v-for="mechanic in mechanics" :key="mechanic.id">
                <td class="py-1">
                  <img
                    src="https://res.cloudinary.com/sigtam/image/upload/v1561254598/icono-mecanico.8013d442_ikia5j.jpg"
                    alt="image"
                  />
                </td>
                <td>{{ mechanic.nombre }} {{ mechanic.apellidos }}</td>
                <td>{{ mechanic.cedula }}</td>
                <td>{{ mechanic.email }}</td>
                <td>{{ mechanic.numeroCelular }}</td>
                <td>
                  <button
                    type="button"
                  ...
                  <button
                    ...
                    data-toggle="modal"
                    id="editar"
                    data-target="#modalMecanico"
                    @click="getMechanic(mechanic._id)"
                  >
                  ...
                  <button
                    ...
                    id="borrar"
                    @click="deleteMechanic(mechanic._id)"
                  >
    ...
    <div
      ...
      tabindex="-1"
      role="dialog"
      aria-labelledby="modalMecanicoTitle"
      aria-hidden="true"
    >
    ...
    <div
      class="modal fade"
      id="modalMecanicoNuevo"
      tabindex="-1"
      role="dialog"
      aria-labelledby="modalMecanicoNuevo"
      aria-hidden="true"
    >
      <div class="modal-dialog" role="document">
        <div class="modal-content">
...
export default {
  ...
  methods: {
    getMechanics() {
      axios.get(`${constants.URL_MECANICOS}`).then(res => {
        this.mechanics = res.data;
      });
    },
    getMechanic(id) {
      axios.get(`${constants.URL_MECANICOS}/${id}`).then(res => {
    ...
    editMechanic() {
      axios
        .put(`${constants.URL_MECANICOS}/${this.idMechanicToEdit}`, {
          nombre: this.nombres,
    ...
    addMechanic() {
      ...
      axios
        .post(`${constants.URL_MECANICOS}/upload`, formData, {
    ...
    deleteMechanic(idMechanic) {
      Swal.fire({
        ...
        if (result.value) {
          axios.delete(`${constants.URL_MECANICOS}/${idMechanic}`).then(res => {
            this.getMechanics();
            console.log(res.data);
          });
          Swal.fire(
