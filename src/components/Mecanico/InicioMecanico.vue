      ...
      <div class="card-body">
          ...
          <div class="col-md-6">
            <div class="form-group row">
              <label class="col-sm-6 col-form-label font-weight-bolder"
                >Seleccione el Cliente</label
              >
              <div class="col-sm-6">
                <select
                  class="form-control"
                  @change="getVehicle(idVehicle)"
                  v-model="idVehicle"
                >
            ...
            >
              <template v-if="vehicle.marca != undefined">
                ...
                >
                  ...
                  <h6 class="font-weight-bold mb-0 ml-2">
        ...
        <div class="row mt-4">
          <div class="col-md-6">
            <div class="form-group row">
              <label class="col-sm-6 col-form-label font-weight-bolder"
                >Seleccione una parte del Automóvil</label
              >
              <div class="col-sm-6">
                <select
                  class="form-control"
                  @change="getPartCart(idPart)"
                  v-model="idPart"
                >
                  <option value="0" selected disabled
                    >Seleccione una Parte</option
                  >
          ...
          <div class="col-md-6">
            <div class="form-group row">
              <label class="col-sm-6 col-form-label font-weight-bolder"
                >Seleccione el Tipo de Servicio</label
              >
              <div class="col-sm-6">
                <select
                  class="form-control"
                  @change="getType(idTipo)"
                  v-model="idTipo"
                >
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="d-flex justify-content-between mb-3 mt-3">
                <h5 class="mt-0 mb-0">
                  Seleccione la Parte de la Carrocería a Reparar
                </h5>
                <div class="custom-control custom-checkbox pl-0">
                  <input
                    ...
                    id="customCheck1"
                    v-model="aniadirCarroceria"
                  />
                  <label
                    class="custom-control-label"
                    style="width: 132px;"
                    for="customCheck1"
                    ><p class="mb-0 text-center">Añadir Parte</p>
                  </label>
                </div>
              </div>
              <div
                class="table-responsive mt-2"
                style="height:300px; overflow-y:scroll; box-shadow: #80808052 2px 3px 7px;"
              >
                        ...
                        <template v-if="!nombre.inFact">
                          <template v-if="!nombre.added">
                            <button
                              ...
                              @click="aniadirServicio(nombre.nombre, nombre.id)"
                            >
                              Añadir
                            </button>
                          </template>
                          <template v-else>
                            <button
                              ...
                              @click="retirarServicio(nombre.id)"
                            >
                              Retirar
                            </button>
                          </template>
            ...
            <div class="col-md-6">
              <template v-if="aniadirCarroceria">
                <h5 class="mt-3 mb-0 text-center">
                  Nueva Parte de la Carrocería
                </h5>
                <div class="mb-2 mt-4">
                  <p class="mt-0 mb-2 font-weight-bolder">
                    Ingrese el nombre de la nueva parte
                  </p>
                    ...
                    <div class="row">
                      <div class="col-md-10">
                        <div class="form-group">
                          <label for="descripcion-reparacion"
                            >Descripción de la reparación</label
                          >
                          <textarea
                            class="form-control"
                            id="descripcion-reparacion"
                            rows="5"
                            @keyup="item.descripcion = $event.target.value"
                          ></textarea>
                        </div>
                      </div>
                      <div class="row ml-2">
                        <div class="col-md-6">
                          <div class="form-group">
                            ...
                            <input
                              ...
                              id="manoObra"
                              placeholder=""
                              @keyup="item.precio = $event.target.value"
                            />
                          </div>
                        </div>
                        <div class="col-md-6">
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group row">
                <label class="col-sm-6 col-form-label font-weight-bold"
                  >Seleccione el sistema del Chasis</label
                >
                <div class="col-sm-6">
                  <select
                    class="form-control"
                    @change="getDataSystem(idSystemSelected)"
                    v-model="idSystemSelected"
                  >
                    <option value="" selected disabled
                      >Seleccione una Sistema</option
                    >
                    <option
                      v-for="system in nombresSistemas"
                      :key="system.nombreSistema"
                      :value="system.idSystem"
                      >{{ system.nombreSistema }}</option
                    >
                  </select>
                </div>
              </div>
            ...
            <div class="col-md-6">
              <template v-if="verSubsistemas">
                <div class="form-group row">
                  <label class="col-sm-6 col-form-label font-weight-bold"
                    >Seleccione el Subsistema</label
                  >
                  <div class="col-sm-6">
                    <select
                      class="form-control"
                      @change="getDataSubSystem(idSubSystemSelected)"
                      v-model="idSubSystemSelected"
                    >
                      <!-- <option value="selected" selected disabled>Seleccione un Subsistema</option>  -->
                      <option
                        v-for="system in sistemas"
                        :key="system.nombre"
                        :value="system.id"
                        >{{ system.nombre }}</option
                      >
                    </select>
                  </div>
                </div>
          ...
          <div class="row mt-3">
                        ...
                        <tr v-for="(item, index) in subsistema" :key="index">
                          ...
                          <td>
                            ...
                            <template v-else>
                              <button
                                class="btn btn-danger btn-sm"
                                @click="retirarSubsistema(item.id)"
                              >
                                Retirar
                              </button>
                            </template>
            ...
            <template v-if="serviciosChasisFactura.length > 0">
              <div class="col-md-6">
                ...
                >
                  <div
                    v-for="(item, index) in serviciosChasisFactura"
                    :key="item.id"
                    :value="index"
                    class="mt-2 ml-2"
                  >
                ...
                <div class="custom-control custom-checkbox pl-0 mt-3">
                  <input
                    ...
                    class="custom-control-input"
                    id="customCheck1"
                    v-model="aniadirProductoChasis"
                  />
                  <label
                    class="custom-control-label"
                    style="width: 152px;"
                    for="customCheck1"
                    ><p class="mb-0 text-center">Añadir Producto</p>
                  </label>
                </div>
              </div>
            </template>
              ...
              <div class="form-group row mt-4">
                ...
                <div class="col-sm-6">
                  <select
                    class="form-control"
                    style="width: 200px"
                    @change="getAllProducts(categoria)"
                    v-model="categoria"
                  >
                    <option
                    ...
                    >
                      ...
                      <td>
                        <button
                          ...
                          @click="
                            aniadirProducto(prod.nombre, prod.precioVenta)
                          "
                        >
                          Añadir
                        </button>
            ...
            <div class="table-responsive mt-4 ">
                    ...
                    <tr v-for="(item, index) in serviciosFactura" :key="index">
                      ...
                      <td>
                        <label
                          ...
                          >{{ item.valueUnity * item.cantidad }} $</label
                        >
                      </td>
                      <template v-if="!generatePdf">
                        <td>
                          <button
                            type="button"
                            class="btn btn-icons btn-rounded btn-success mr-1"
                            @click="openModalEditService(item)"
                          >
                            <i
                              class="mdi mdi-file-document-edit-outline mdi-18px"
                            ></i>
                          </button>
                          <button
                            type="button"
                          ...
                          >
                            <i
                              class="mdi mdi-delete-forever-outline mdi-18px"
                            ></i>
                          </button>
                        </td>
                      </template>
              ...
              <div class="d-flex justify-content-between mt-3">
                <template v-if="confirmChanges">
                  <button class="btn btn-primary" @click="confirmarCambios()">
                    Confirmar Cambios
                  </button>
                </template>
                <template v-if="generatePdf">
        ...
        <div
          ...
          tabindex="-1"
          role="dialog"
          aria-labelledby="modalClientNuevo"
          aria-hidden="true"
        >
          <div class="modal-dialog " role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h3 class="modal-title" id="modalClientNuevo">
                  Añadir Cliente
                </h3>
                <button
                  type="button"
                      ...
                      >
                        <template v-if="!addCar">
                          ...
                          <div class="row">
                            ...
                            <div class="form-group col-md-8">
                              <div
                                class="form-group d-flex flex-row justify-content-between mt-4"
                              >
                                <button
                                  type="button"
                                  @click="addCar = true"
                                  class="btn btn-success btn-fw mt-4git log btn-block"
                                >
                                  <i class="mdi mdi-car-hatchback"></i>
                                  <template v-if="addDataNewCar"
                                    >Editar Datos del Vehiculo</template
                                  >
                                  <template v-else
                                    >Añadir Vehiculo</template
                                  >
                                </button>
                              </div>
                            </div>
                                ...
                                <div class="col-md-12">
                                  ...
                                  <div class="row">
                                    <div class="form-group col-md-6">
                                      ...
                                      <select
                                        class="form-control"
                                        @change="viewMarcas(marcaselected)"
                                    ...
                                    <div class="form-group col-md-6">
                                      ...
                                      <select
                                        class="form-control"
                                        @change="selectModel(modelselected)"
                                        v-model="modelselected"
                                      >
                                        <option value selected disabled
                                          >Seleccione un Modelo</option
                                        >
                                        <option
                                          v-for="(marca, key) in modelos"
                                          :key="key"
                                          >{{ marca }}</option
                                        >
                                      </select>
                                  ...
                                  <div class="row">
                                    ...
                                    <div class="form-group col-md-6">
                                      ...
                                      <input
                                        type="text"
                                        class="form-control"
                                        v-model="nuevamarcallantasCarro"
                                      />
                                    </div>
                                  </div>
                                </div>
                                ...
                                <div class="col-md-12 mt-4">
                                  ...
                                  >
                                    <button
                                      type="button"
                                      @click="addCar = false"
                                      class="btn btn-secondary btn-fw mr-2"
                                    >
                                      <i class="mdi mdi-file-document"></i
                                      >Cancelar
                                    </button>
        ...
        <div
          ...
          tabindex="-1"
          role="dialog"
          aria-labelledby="modalEditServiceLabel"
          aria-hidden="true"
        >
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">
                  Editar detalles de la reparación
                </h5>
                <button
                  type="button"
                  class="close"
...
export default {
  ...
  data() {
    return {
      ...
      marcallantasCarro: "",
      nuevaplacaCarro: "",
      nuevakilometrajeCarro: "",
      nuevaanioCarro: "",
      nuevacolorCarro: "",
      nuevamotorCarro: "",
      nuevamarcallantasCarro: "",
      descripcionServicio: "",
      datosChasis: [],
      precioServicio: "",
      partsAdd: [],
  ...
  methods: {
    ...
    getMarcas() {
      axios.get(`${constants.URL_MARCAS}/all`).then(res => {
        this.marcas = res.data;
      });
    },
    viewMarcas(marca) {
      // console.log(`marca Seleccionada: ${marca}`);
      axios.get(`${constants.URL_MARCAS}/modelos/${marca}`).then(res => {
    ...
    getAllTypesServices() {
      axios.get(`${constants.URL_SERVICIOS}`).then(res => {
        this.tiposServicios = res.data;
      });
    },

    getAllClients() {
      axios.get(`${constants.URL_CLIENTES}`).then(res => {
        this.clientes = res.data;
      });
    },

    getAllVehicles() {
      axios.get(`${constants.URL_VEHICULOS}`).then(res => {
    ...
    aniadirSubsistema(nombresubsistema, indice) {
      ...
      this.serviciosChasisFactura.push({
        id: indice,
        nombreParte: nombresubsistema,
        descripcion: "",
        precio: ""
      });
    },
    ...
    getDataCarrocerias() {
      axios.get(`${constants.URL_CARROCERIA}`).then(res => {
        // console.log(res.data)
        this.datosCarroceria = res.data;
        this.datosCarroceria.forEach(dato => {
          this.nombresPartes.push({
            id: dato.id,
            nombre: dato.nombre,
            precio: dato.precio,
            added: false,
    ...
    getDataChasis() {
      axios.get(`${constants.URL_CHASIS}`).then(res => {
        this.datosChasis = res.data;
        this.datosChasis.forEach(dato => {
          this.nombresSistemas.push({
            idSystem: dato.id,
            nombreSistema: dato.nombre,
            added: false,
            inFact: false
          });
        });
    ...
    generarFactura2() {
      ...
      doc.text(
        40,
        120,
        `Cliente: ${this.nombreSeleccionado} ${this.apellidoSeleccionado}`
      );
      doc.text(40, 140, `N° Identificación: ${this.cedulaSeleccionada}`);
      doc.text(
        40,
        180,
        `Automóvil: ${this.vehicle.marca} ${this.vehicle.modelo}`
      );
    ...
    addClient() {
      axios
        ...
        .then(res => {
          ...
          Swal.fire(
            "Cliente Agregado",
            "Se realizó la inserción con éxito ",
            "success"
          );
